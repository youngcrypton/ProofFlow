...
The current file store is local disk, not a multi-region object-storage system. Production evidence uploads are unavailable until an approved malware scanner is integrated. Uploads fail closed with `SCANNER_UNAVAILABLE`, remain quarantined, and are never promoted to clean storage.
