...
const ProofNetworkField: FC<ProofNetworkFieldProps> = ({
  ...
  const buildGrid = useCallback(() => {
    ...
    const height = Math.max(1, Math.floor(wrap.getBoundingClientRect().height));
  ...
  return (
    <section className={`proof-network-field ${className}`} style={style}>
      <div ref={wrapperRef} className="proof-network-field__wrap">
        <canvas ref={canvasRef} className="proof-network-field__canvas" />
      </div>
    </section>
  );
};
