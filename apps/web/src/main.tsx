...
function App() {
  ...
  return <div className="app-shell phase3-app">
    ...
    <main id="main-content" className="main-content">
      ...
      <section className="page-heading hero-stage"><div className="hero-glow" aria-hidden="true" /><div className="hero-copy"><div className="eyebrow">Trust execution console · {statusLabel}</div><h2>Make the next safe move.</h2><p>AI checks the evidence. Policy decides. You approve. X Layer settles.</p><div className="hero-proof"><span className="hero-proof-dot" aria-hidden="true" /><span>Every release is reviewable before it is irreversible.</span></div></div><div className="heading-actions"><button className="button secondary" disabled={resetting} onClick={() => void resetDemo()}>{resetting ? "Resetting…" : "Reset demo"}</button><button className="button primary" onClick={() => setCreateOpen(true)}>+ Create agreement <span aria-hidden="true">↗</span></button></div></section>
...
function Metric({ label, value, detail, tone = "" }: { label: string; value: string; detail: string; tone?: string }) { return <article className={`metric ${tone}`}><span className="metric-label">{label}</span><strong>{value}</strong><small>{detail}</small><span className="metric-sheen" aria-hidden="true" /></article>; }
...
function EmptyState({ title, copy }: { title: string; copy: string }) { return <div className="empty-state"><span className="empty-orbit" aria-hidden="true">○</span><b>{title}</b><p>{copy}</p></div>; }
