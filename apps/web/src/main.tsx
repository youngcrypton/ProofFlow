...
function App() {
  ...
  const pageContent = activeView === "overview" ? <>
    ...
    <section className="protocol-section" aria-labelledby="why-proofflow"><div className="protocol-section-header"><span className="eyebrow">Protocol primitives</span><h3 id="why-proofflow">Why trust execution through ProofFlow?</h3><p>Every milestone follows the same accountable path: requirement, evidence, policy, authorization, settlement, receipt.</p></div><div className="protocol-feature-grid"><article className="protocol-feature-card"><span>01 · EVIDENCE</span><h4>Evidence before release</h4><p>Payment eligibility depends on submitted and reviewed evidence—not an unverified promise.</p></article><article className="protocol-feature-card"><span>02 · POLICY</span><h4>Deterministic policy gates</h4><p>Release conditions are evaluated explicitly, with blocked reasons kept visible for review.</p></article><article className="protocol-feature-card"><span>03 · RECEIPT</span><h4>Auditable settlement</h4><p>Authorization, transaction state, receipt reconciliation, and audit events remain connected.</p></article></div></section>
