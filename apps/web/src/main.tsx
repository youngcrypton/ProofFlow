...
type AgreementDraft = { title: string; description: string; acceptanceCriteria: string; payer: string; recipient: string; tokenAddress: string; amountBaseUnits: string; deadline: string; evidenceTypes: EvidenceType[]; minimumConfidenceBps: number; releaseAmountBaseUnits: string; policyVersion: string };
...
function workspaceCopy(role: WorkspaceRole) {
  return role === "client"
    ? { title: "Client command center", description: "Create agreements, review evidence, and authorize settlements.", assigned: "My agreements", emptyTitle: "No agreements yet", emptyCopy: "Create your first agreement to start a verifiable commitment." }
    : { title: "Contractor workspace", description: "View assigned agreements, submit evidence, and track settlement.", assigned: "Assigned to me", emptyTitle: "No agreements assigned yet", emptyCopy: "When a client creates an agreement for your wallet, it will appear here." };
}
...
function App() {
  ...
  const [workspaceRole, setWorkspaceRole] = useState<WorkspaceRole | null>(() => readWorkspaceRole());
  const [roleOnboardingOpen, setRoleOnboardingOpen] = useState(() => getViewFromHash() !== "landing" && readWorkspaceRole() === null);

  function chooseWorkspaceRole(role: WorkspaceRole) {
    persistWorkspaceRole(window.localStorage, role); setWorkspaceRole(role);
    setRoleOnboardingOpen(false);
  }

  const workspaceAgreements = useMemo(() => {
    if (!workspaceRole || !walletAddress) return [];
    return agreements.filter((agreement) => agreementMatchesWorkspace(agreement, workspaceRole, walletAddress));
  }, [agreements, walletAddress, workspaceRole]);
  const roleCopy = workspaceRole ? workspaceCopy(workspaceRole) : null;

  const loadAgreements = useCallback(async () => {
    if (!workspaceRole || !walletAddress || !walletSessionReady) {
      setAgreements([]);
      setSelectedId(null);
      setLoading(false);
      return;
    ...
    setLoading(true);
    try {
      const query = workspaceQuery(workspaceRole, walletAddress);
      const result = await api<Agreement[]>(`/api/v1/agreements${query}`);
  ...
  }, [walletAddress, walletSessionReady, workspaceRole]);
  ...
  useEffect(() => {
    if (!workspaceRole || !walletAddress || !walletSessionReady) return;
    void loadAgreements();
  }, [loadAgreements, walletAddress, walletSessionReady, workspaceRole]);
  useEffect(() => {
  ...
  const visibleAgreements = workspaceRole && walletAddress && walletSessionReady ? workspaceAgreements : [];
  const selected = detail?.agreement && visibleAgreements.some((item) => item.id === detail.agreement.id) ? detail.agreement : visibleAgreements.find((item) => item.id === selectedId) ?? null;
  const attention = visibleAgreements.filter((item) => ["BLOCKED", "DISPUTED", "UNDER_REVIEW", "EVIDENCE_SUBMITTED", "AWAITING_FUNDING"].includes(item.state)).length;
  const reviewQueue = visibleAgreements.filter((item) => ["UNDER_REVIEW", "EVIDENCE_SUBMITTED"].includes(item.state)).length;
  const escrow = visibleAgreements.filter((item) => item.state !== "RELEASED").reduce((sum, item) => sum + BigInt(item.amountBaseUnits || "0"), 0n);
  ...
  }, [visibleAgreements, searchQuery, stateFilter]);
  const reviewAgreements = useMemo(() => sortPriority(visibleAgreements.filter((agreement) => ["EVIDENCE_SUBMITTED", "UNDER_REVIEW", "REVIEWED", "READY_TO_RELEASE"].includes(agreement.state))), [visibleAgreements]);
  ...
  async function handleCreated(agreement: Agreement) { setCreateOpen(false); setNotice({ kind: "success", text: `Agreement ${agreement.id} created. Vault setup is pending.` }); await loadAgreements(); setSelectedId(agreement.id); await loadDetail(agreement.id); }
  function openCreateAgreement() {
    if (!walletAddress || !walletSessionReady || !walletSession.hasValidSession(walletAddress)) {
      setNotice({ kind: "info", text: "Connect and sign your wallet access message before publishing an agreement." });
      void connectWallet();
      return;
    }
    setCreateOpen(true);
  ...
  async function fundAgreement(agreementId: string) {
    const provider = asEip1193Provider(appKitProvider);
    if (!provider) { setWalletError("Wallet provider unavailable. Reconnect before funding."); return; }
    if (!walletAddress) { await connectWallet(); return; }
    setWalletBusy(true); setWalletError(null);
    try {
      const preview = await api<ChainPreview>(`/api/v1/agreements/${agreementId}/chain-preview`);
      ...
      for (let attempt = 0; attempt < 30 && !verified; attempt += 1) {
        try {
          await api(`/api/v1/agreements/${agreementId}/fund`, { method: "POST", body: JSON.stringify({ transactionHash }) });
          verified = true;
        } catch (error) {
      ...
      setNotice({ kind: "success", text: `Vault funding verified on X Layer: ${shortHash(transactionHash)}.` });
      await refresh();
    } catch (error) { setWalletError(error instanceof Error ? error.message : "Vault funding could not be verified."); }
    finally { setWalletBusy(false); }
  }
  ...
  async function simulate(action: "fund" | "evidence" | "review" | "release") {
    ...
    if (action === "evidence") {
      if (workspaceRole !== "contractor") { setNotice({ kind: "danger", text: "Only the assigned contractor can submit evidence." }); return; }
      setEvidenceOpen(true); return;
    }
    if (!selectedId) return;
    if (action === "fund") { await fundAgreement(selectedId); return; }
    const paths: Record<"review", [string, RequestInit]> = { review: [`/api/v1/agreements/${selectedId}/review`, { method: "POST", body: JSON.stringify({ evidenceText: "Evidence received for the milestone." }) }] };
  ...
  const pageTitle = activeView === "overview" ? roleCopy?.title ?? "Workspace" : activeView === "agreements" ? roleCopy?.assigned ?? "Agreements" : activeView === "review" ? "Review queue" : activeView === "activity" ? "Activity" : activeView === "wallet" ? "Wallet" : "Settings";
  const pageDescription = activeView === "overview" ? roleCopy?.description ?? "Choose a workspace role, then connect your wallet to load agreements securely." : activeView === "agreements" ? "Open the agreements authorized for the connected wallet and selected role." : activeView === "review" ? "Make the next decision with evidence, policy, and uncertainty in view." : activeView === "activity" ? "A verifiable record of every meaningful workflow event." : activeView === "wallet" ? "Review network health and keep authorization at the human boundary." : "Control the workspace environment without changing the protocol rules.";
  const pageContent = activeView === "overview" ? <>
    {!workspaceRole || !walletAddress ? <section className="workspace-gate panel"><div className="workspace-gate-mark">{workspaceRole ? "◈" : "01"}</div><div><span className="eyebrow">{workspaceRole ? "Wallet identity required" : "Choose your workspace"}</span><h2>{workspaceRole ? "Connect your wallet to load your workspace." : "How are you using ProofFlow?"}</h2><p>{workspaceRole ? "Your wallet identifies the agreements you created or were assigned. ProofFlow will only load matching agreements." : "Choose a role now. You can switch between Client and Contractor at any time without reconnecting."}</p><div className="workspace-gate-actions">{!workspaceRole && <><button className="button primary" onClick={() => chooseWorkspaceRole("client")}>Client workspace</button><button className="button secondary" onClick={() => chooseWorkspaceRole("contractor")}>Contractor workspace</button></>}{workspaceRole && <button className="button primary" onClick={() => void connectWallet()}>Connect wallet</button>}</div></div></section> : null}
    <section className="page-heading hero-stage"><ProofNetworkField className="hero-network-field" dotSize={2} gap={32} baseColor="#536272" activeColor="#7dd3fc" proximity={120} /><TrustGrid /><div className="hero-glow" aria-hidden="true" /><AnimatedContent className="hero-copy"><div className="eyebrow">Verifiable work · programmable trust · {statusLabel}</div><h2><BlurText>Trust the evidence.<br />Not the promise.</BlurText></h2><p>ProofFlow verifies milestone evidence, enforces deterministic release policies, and prepares settlement on X Layer.</p><div className="hero-proof"><span className="hero-proof-dot" aria-hidden="true" /><span>Every release is reviewable before it is irreversible.</span></div></AnimatedContent><div className="heading-actions"><button className="button secondary" onClick={() => navigate("agreements")}>Open console <span aria-hidden="true">→</span></button><button className="button primary" onClick={openCreateAgreement}>Create agreement <span aria-hidden="true">↗</span></button></div></section>
    ...
    <section className="metric-grid motion-stagger" aria-label="Workspace summary"><Metric label={workspaceRole === "contractor" ? "Assigned to me" : "My agreements"} value={String(visibleAgreements.length)} detail={workspaceRole === "contractor" ? "Agreements for this wallet" : "Agreements created by this wallet"} numeric={visibleAgreements.length} /><Metric label={workspaceRole === "contractor" ? "Needs action" : "Needs your review"} value={String(attention)} detail="Next steps in this workspace" tone={attention > 0 ? "lime" : ""} numeric={attention} /><Metric label={workspaceRole === "contractor" ? "Pending settlement" : "Ready for settlement"} value={`${formatUnits(escrow)} XLAY`} detail="Unsettled agreement value" tone="dark" /><Metric label="Network health" value={network ? "Online" : "Offline"} detail={network ? "X Layer RPC responding" : "Last check unavailable"} /></section>
    ...
    <section className="dashboard-grid overview-grid"><div className="panel priority-panel"><PanelHeading title="Priority queue" kicker="What needs attention first" action={<button className="text-button" onClick={() => navigate("review")}>Open review queue →</button>} /><div className="queue-list">{loading ? <SkeletonRows /> : !walletAddress ? <EmptyState title="Connect your wallet to load your workspace." copy="Your wallet identifies the agreements you created or were assigned." action={<button className="button primary" onClick={() => void connectWallet()}>Connect wallet</button>} /> : visibleAgreements.length === 0 ? <EmptyState title={roleCopy?.emptyTitle ?? "No agreements yet"} copy={roleCopy?.emptyCopy ?? "Choose a workspace role to continue."} action={workspaceRole === "client" ? <button className="button primary" onClick={openCreateAgreement}>Create agreement</button> : undefined} /> : sortPriority(visibleAgreements).slice(0, 4).map((agreement) => <QueueRow key={agreement.id} agreement={agreement} selected={agreement.id === selectedId} onClick={() => { setSelectedId(agreement.id); navigate("agreements"); }} />)}</div></div><div className="panel network-panel"><PanelHeading title="Network health" kicker="Observed now" action={<button className="network-action" onClick={() => void loadNetwork()}>Refresh ↻</button>} /><div className="network-hero"><div className="network-orbit"><span>×</span></div><div><strong>{network ? "Connected" : "Unavailable"}</strong><p>{network ? `X Layer ${network.chainId === 1952 ? "testnet" : "mainnet"}` : networkError ?? "RPC status is being checked."}</p></div></div><div className="network-detail"><span>Chain ID</span><code>{network?.chainId ?? "—"}</code><span>Latest block</span><code>{network?.blockNumber ?? "—"}</code></div></div></section>
    <section className="panel recent-panel"><PanelHeading title={roleCopy?.assigned ?? "Agreements"} kicker={`${visibleAgreements.length} in workspace`} action={<button className="text-button" onClick={() => navigate("agreements")}>View all →</button>} /><div className="recent-grid">{loading ? <SkeletonRows /> : !walletAddress ? <EmptyState title="Connect your wallet to load your workspace." copy="Your wallet identifies the agreements you created or were assigned." action={<button className="button primary" onClick={() => void connectWallet()}>Connect wallet</button>} /> : visibleAgreements.length ? visibleAgreements.slice(0, 3).map((agreement) => <button className="recent-card" key={agreement.id} onClick={() => { setSelectedId(agreement.id); navigate("agreements"); }}><span className={`state-mark ${stateTone(agreement.state)}`}>{stateIcon(agreement.state)}</span><span><b>{agreement.title}</b><small>{workspaceRole === "client" ? shortAddress(agreement.recipient) : shortAddress(agreement.payer)} · {relativeTime(agreement.updatedAt)}</small></span><strong>{formatUnits(agreement.amountBaseUnits)} <small>XLAY</small></strong></button>) : <EmptyState title={roleCopy?.emptyTitle ?? "No agreements yet"} copy={roleCopy?.emptyCopy ?? "Choose a workspace role to continue."} action={workspaceRole === "client" ? <button className="button primary" onClick={openCreateAgreement}>Create agreement</button> : undefined} />}</div></section>
  </> : activeView === "agreements" ? <>
    <section className="page-heading compact-heading"><div><div className="eyebrow">Workspace index · {filteredAgreements.length} matching</div><h2>Agreements <em>workspace</em></h2><p>Searchable commitments with their evidence, policy, and settlement state.</p></div><div className="heading-actions"><button className="button primary" onClick={openCreateAgreement}>+ Create agreement</button></div></section>
  ...
  if (activeView === "landing") return <>
    <LandingPage statusLabel={statusLabel} network={network} agreements={agreements} walletAddress={walletAddress} walletBusy={walletBusy} walletStatus={walletStatus} walletError={walletError} onNavigate={navigate} onCreate={openCreateAgreement} onConnect={() => void connectWallet()} />
    {createOpen && <CreateAgreementModal onClose={() => setCreateOpen(false)} onCreated={handleCreated} walletAddress={walletAddress} />}
  </>;

  if (roleOnboardingOpen) return <RoleOnboarding onChoose={chooseWorkspaceRole} />;

  return <div className="app-shell phase3-app">
    <a className="skip-link" href="#main-content">Skip to main content</a>
    <Sidebar activeView={activeView} network={statusLabel} walletAddress={walletAddress} workspaceRole={workspaceRole} onRoleChange={(role) => chooseWorkspaceRole(role)} mobileOpen={mobileNavOpen} onToggle={() => setMobileNavOpen((value) => !value)} onConnect={() => void connectWallet()} onCreate={() => { openCreateAgreement(); setMobileNavOpen(false); }} onNavigate={navigate} />
    <main id="main-content" className="main-content">
      <WorkspaceHeader workspaceRole={workspaceRole} walletAddress={walletAddress} walletChainId={walletChainId} mobileNavOpen={mobileNavOpen} onRoleChange={chooseWorkspaceRole} onConnect={() => void connectWallet()} onToggle={() => setMobileNavOpen((value) => !value)} />
...
function WorkspaceSwitcher({ role, onChange }: { role: WorkspaceRole | null; onChange: (role: WorkspaceRole) => void }) {
  const current = role ? workspaceRoleLabel(role) : "Choose workspace";
  return <label className="workspace-select"><span className="workspace-avatar">{role ? current[0] : "W"}</span><span><b>{current} workspace</b><small>Switch role without reconnecting</small></span><select aria-label="Choose workspace role" value={role ?? ""} onChange={(event) => { const value = event.target.value; if (value === "client" || value === "contractor") onChange(value); }}><option value="" disabled>Choose role</option><option value="client">Client Workspace</option><option value="contractor">Contractor Workspace</option></select></label>;
}

function RoleOnboarding({ onChoose }: { onChoose: (role: WorkspaceRole) => void }) {
  return <div className="role-modal-backdrop" role="presentation"><section className="role-modal" role="dialog" aria-modal="true" aria-labelledby="role-modal-title"><span className="eyebrow">ProofFlow workspace</span><h2 id="role-modal-title">How are you using ProofFlow?</h2><p>Choose the view that matches your work. You can switch roles any time without reconnecting your wallet.</p><div className="role-choice-grid"><button type="button" className="role-choice" onClick={() => onChoose("client")}><strong>Client</strong><span>Create agreements, define release conditions, review evidence, and authorize settlements.</span><b>Use Client workspace →</b></button><button type="button" className="role-choice" onClick={() => onChoose("contractor")}><strong>Contractor</strong><span>View assigned agreements, complete work, submit evidence, and track settlement.</span><b>Use Contractor workspace →</b></button></div></section></div>;
}

function WorkspaceHeader({ workspaceRole, walletAddress, walletChainId, mobileNavOpen, onRoleChange, onConnect, onToggle }: { workspaceRole: WorkspaceRole | null; walletAddress: string | null; walletChainId: number | null; mobileNavOpen: boolean; onRoleChange: (role: WorkspaceRole) => void; onConnect: () => void; onToggle: () => void }) {
  const workspaceName = workspaceRole ? `${workspaceRoleLabel(workspaceRole)} Workspace` : "Choose workspace";
  const walletLabel = walletAddress ? shortAddress(walletAddress) : "Connect Wallet";
  const walletState = walletAddress && walletChainId !== XLAYER_TESTNET_CHAIN_ID ? "Wrong network" : walletAddress ? "Wallet ready" : "Required to settle";
  return <header className="topbar workspace-header">
    <div className="header-brand"><span className="brand-mark">P</span><b>ProofFlow</b></div>
    <div className="header-context"><span className="header-eyebrow">Current workspace</span><h1>{workspaceName}</h1></div>
    <div className="header-controls"><WorkspaceSwitcher role={workspaceRole} onChange={onRoleChange} /><button className={`header-wallet ${walletAddress ? "connected" : ""}`} type="button" onClick={onConnect} aria-label={walletAddress ? `Connected wallet ${walletLabel}` : "Connect wallet"}><span className="wallet-chip-icon">◈</span><span><b>{walletLabel}</b><small>{walletState}</small></span></button><button className="mobile-header-menu" type="button" aria-label={mobileNavOpen ? "Close navigation" : "Open navigation"} aria-expanded={mobileNavOpen} onClick={onToggle}>{mobileNavOpen ? "×" : "☰"}</button></div>
  </header>;
}

function Sidebar({ activeView, network, walletAddress, workspaceRole, onRoleChange, mobileOpen, onToggle, onConnect, onCreate, onNavigate }: { activeView: View; network: string; walletAddress: string | null; workspaceRole: WorkspaceRole | null; onRoleChange: (role: WorkspaceRole) => void; mobileOpen: boolean; onToggle: () => void; onConnect: () => void; onCreate: () => void; onNavigate: (view: View) => void }) {
  const items: { view: View; icon: string; label: string }[] = [{ view: "overview", icon: "⌂", label: "Overview" }, { view: "agreements", icon: "▣", label: "Agreements" }, { view: "review", icon: "◌", label: "Review queue" }, { view: "activity", icon: "≡", label: "Activity" }];
  useEffect(() => {
    if (!mobileOpen) return;
    const sidebar = document.querySelector<HTMLElement>(".sidebar.mobile-open");
    const focusable = () => Array.from(sidebar?.querySelectorAll<HTMLElement>("button, a, [tabindex]:not([tabindex='-1'])") ?? []).filter((element) => !element.hasAttribute("disabled"));
    focusable()[0]?.focus();
...
function QueueRow({ agreement, selected, onClick }: { agreement: Agreement; selected: boolean; onClick: () => void }) { return <SpotlightCard interactive ariaLabel={`Open agreement ${agreement.title}`} className={`queue-row ${selected ? "selected" : ""}`} onClick={onClick}><span className={`state-mark ${stateTone(agreement.state)}`}>{stateIcon(agreement.state)}</span><span className="queue-main"><b>{agreement.title}</b><small>{agreement.id} · {agreement.policy.requiredEvidence.length} evidence requirement{agreement.policy.requiredEvidence.length === 1 ? "" : "s"}</small></span><span className="queue-action">{priorityReason(agreement.state)}</span><span className="queue-amount">{formatUnits(agreement.amountBaseUnits)} XLAY</span><span className="queue-date">{relativeTime(agreement.updatedAt)}</span></SpotlightCard>; }
...
function EmptyState({ title, copy, action }: { title: string; copy: string; action?: ReactNode }) { return <div className="empty-state"><span className="empty-orbit" aria-hidden="true">○</span><b>{title}</b><p>{copy}</p>{action ?? null}</div>; }
...
function CreateAgreementModal({ onClose, onCreated, walletAddress }: { onClose: () => void; onCreated: (agreement: Agreement) => Promise<void>; walletAddress: string | null }) {
  ...
  const [draft, setDraft] = useState<AgreementDraft>({ title: "", description: "", acceptanceCriteria: "", payer: walletAddress ?? "", recipient: "", tokenAddress: nativeTokenAddress, amountBaseUnits: "", deadline: "", evidenceTypes: [], minimumConfidenceBps: 9000, releaseAmountBaseUnits: "", policyVersion: "proof-v1" });
  ...
  function validateCurrentStep(): boolean {
    ...
    if (step === 1) {
      ...
      if (!draft.acceptanceCriteria.trim()) next.acceptanceCriteria = "Define how the client will determine that the task is complete.";
    }
    if (step === 2) {
  ...
  return <Modal title="Create your agreement" eyebrow={`New agreement · step ${step} of 4`} onClose={onClose}>
    ...
    <form className={`modal-form agreement-wizard-form wizard-step-${step}`} onSubmit={(event) => void (step === 4 ? publish(event) : undefined)} noValidate>
      {step === 1 && <>
        ...
        <Field id="agreement-acceptance" label="Acceptance criteria" value={draft.acceptanceCriteria} placeholder="List the concrete conditions the contractor must satisfy." helper="These terms remain locked with the agreement." error={fieldErrors.acceptanceCriteria} textarea onChange={(value) => update("acceptanceCriteria", value)} required />
      ...
      {step === 4 && <>
        ...
        <ReviewSection title="Agreement" onEdit={() => setStepAndFocus(1)}><InfoRow label="Name" value={body.title} /><InfoRow label="Description" value={body.description} /><InfoRow label="Acceptance" value={body.acceptanceCriteria} /><InfoRow label="Deadline" value={formatDate(body.deadline)} /></ReviewSection>
...
function EvidenceModal({ agreement, onClose, onSubmitted }: { agreement: Agreement; onClose: () => void; onSubmitted: () => Promise<void> }) {
  const [file, setFile] = useState<File | null>(null); const [link, setLink] = useState(""); const [explanation, setExplanation] = useState(""); const [notes, setNotes] = useState(""); const [type, setType] = useState<EvidenceType>(agreement.policy.requiredEvidence[0] ?? "status_update"); const [busy, setBusy] = useState(false); const [error, setError] = useState<string | null>(null);
  async function submit(event: FormEvent) {
    event.preventDefault();
    if (!file && !/^https?:\/\//i.test(link.trim())) { setError("Choose a file or enter a valid document link."); return; }
    setBusy(true); setError(null);
    try {
      if (file) {
        const form = new FormData(); form.set("evidenceType", type); form.set("submittedBy", agreement.recipient); form.set("file", file);
        await api(`/api/v1/agreements/${agreement.id}/evidence/upload`, { method: "POST", body: form, headers: {} });
      } else {
        ...
        await api(`/api/v1/agreements/${agreement.id}/evidence`, { method: "POST", body: JSON.stringify({ agreementId: agreement.id, submittedBy: agreement.recipient, submittedAt: new Date().toISOString(), explanation: explanation.trim(), notes: notes.trim(), items: [{ type, name: new URL(link.trim()).pathname.split("/").filter(Boolean).at(-1) ?? "linked-evidence", mediaType: "text/uri-list", sha256: digest, uri: link.trim() }] }) });
      }
      await onSubmitted();
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Evidence could not be submitted."); }
    finally { setBusy(false); }
  }
  return <Modal title="Submit evidence" eyebrow={`${agreement.title} · contractor evidence`} onClose={onClose}><form className="modal-form" onSubmit={(event) => void submit(event)}><Field label="Evidence type" value={type} select options={agreement.policy.requiredEvidence} onChange={(value) => setType(value as EvidenceType)} /><label className="field"><span>Evidence file</span><input type="file" onChange={(event) => setFile(event.target.files?.[0] ?? null)} /></label><Field label="Document link" value={link} placeholder="https://…" helper="Use a stable link when the evidence is hosted elsewhere." onChange={setLink} /><Field label="Written explanation" value={explanation} textarea helper="Explain how this evidence satisfies the requested requirement." onChange={setExplanation} /><Field label="Additional notes" value={notes} textarea helper="Optional context for the client." onChange={setNotes} /><div className="upload-note"><b>Controlled intake</b><p>Files remain subject to the configured scanner. Links and explanations are recorded in the immutable evidence manifest and never treated as trusted instructions.</p></div>{error && <div className="form-error" role="alert">{error}</div>}<ModalActions onClose={onClose} busy={busy} submitLabel="Submit evidence" /></form></Modal>;
}
