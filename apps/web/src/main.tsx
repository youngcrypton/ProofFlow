...
function App() {
  ...
  return <div className="app-shell">
    ...
    <main className="main-content">
      ...
      <section className="page-heading"><div><div className="eyebrow">Trust execution console</div><h2>Overview</h2><p>Evidence, policy, and settlement in one accountable loop.</p></div><button className="button primary" onClick={() => setNotice({ kind: "info", text: "Agreement creation is next in the workflow. The live API foundation is ready." })}>+ Create agreement</button></section>
...
function PanelHeading({ title, kicker, action }: { title: string; kicker: string; action?: React.ReactNode }) { return <div className="panel-heading"><div><span>{kicker}</span><h3>{title}</h3></div>{action}</div>; }
