  ...
  return <div className="app-shell phase3-app">
    ...
    <main id="main-content" className="main-content">
      <header className="topbar">
        <div className="mobile-workspace-context"><span>Workspace</span><strong>{workspaceRole ? workspaceRoleLabel(workspaceRole) : "Choose role"}</strong></div>
        <button className="mobile-header-menu" type="button" aria-label={mobileNavOpen ? "Close navigation" : "Open navigation"} aria-expanded={mobileNavOpen} onClick={() => setMobileNavOpen((value) => !value)}>☰</button>
...
function WorkspaceSwitcher({ role, onChange }: { role: WorkspaceRole | null; onChange: (role: WorkspaceRole) => void }) {
  return <label className="workspace-select"><span className="workspace-avatar">{role === "contractor" ? "C" : "C"}</span><span><b>{role ? `${workspaceRoleLabel(role)} workspace` : "Choose workspace"}</b><small>Switch role without reconnecting</small></span><select aria-label="Choose workspace role" value={role ?? ""} onChange={(event) => { const value = event.target.value; if (value === "client" || value === "contractor") onChange(value); }}><option value="" disabled>Choose role</option><option value="client">Client Workspace</option><option value="contractor">Contractor Workspace</option></select></label>;
}
